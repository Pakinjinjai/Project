...
export default {
  ...
  methods: {
    ...
    async fetchDataFromApi(searchQuery) {
      try {
        const res = await axios.get(`${baseURL}/users/?Search=${searchQuery}`);
    ...
    async delUser(_id) {
      try {
        ...
        if (confirmResult) {
          const accessToken = localStorage.getItem("accessToken");
          const res = await axios.delete(`${baseURL}/queues/${_id}`, {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
    ...
    async addQueue_Btn(user) {
      try {
        ...
        // เรียกใช้งานเพื่อดึงข้อมูลผู้ใช้งานทั้งหมดอีกครั้งเพื่ออัปเดตข้อมูลในตาราง
        this.getAllUser();
      } catch (error) {
    ...
    UpdateModal(user) {
      ...
      console.log(
        "ค่า status ที่ได้จาก API response:",
    ...
    async UpdateQueue_Btn() {
      try {
        ...
        const res = await axios.patch(
          `${baseURL}/queues/update/${this.SelectedItem._id}`,
          {
        ...
        <div class="overflow-x-auto">
          <table id="dataTable" class="w-full text-sm text-left text-gray-500">
            ...
            <tbody v-if="Queue.length > 0">
              <tr
                v-for="(item, index) in sorteduser.slice(startIndex, endIndex)"
                :key="item._id"
                :class="{
                  'bg-white': index % 2 === 0,
                  'bg-[#F6F6F6]': index % 2 !== 0,
                ...
                <td class="px-4 py-3 flex text-[#303030] justify-center">
                  ...
                  <button
                    ...
                    type="button">
                    <svg
                      class="w-[16px] h-[16px] text-gray-800"
                      aria-hidden="true"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 18 18">
                      ...
                      <div class="p-4 bg-white rounded-lg shadow sm:p-5">
                        ...
                        <div class="flex rounded-t border-b sm:mb-2">
                          <div class="flex gap-4">
                            <h3
                              class="text-lg text-center font-semibold text-[#303030]">
                              รายการคิวรายบุคคล
                            </h3>
                            <button
                              @click="trueInfoModal(item)"
                              type="button"
                        ...
                        <div class="overflow-x-auto rounded-lg border">
                          <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody v-if="SelecttrueQueue.length > 0">
                              <tr
                                v-for="(item, index) in sortedQueuemefalse"
                                :key="item._id"
                                :class="{
                                  'bg-white': index % 2 === 0,
                                  'bg-[#F6F6F6]': index % 2 !== 0,
                                ...
                                <td
                                  class="px-4 py-3 flex text-[#303030] justify-center">
                                  <!--Info_Btn-->
                                  <button
                                    @click="trueQueueModal(item)"
                  ...
                  <div
                    ...
                    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                      ...
                      <div class="p-4 bg-white rounded-lg shadow sm:p-5">
                        <!-- Modal header -->
                        <div
                          class="flex justify-center rounded-t border-b sm:mb-2">
                          <div class="flex gap-4">
                            <h3
                              class="text-lg text-center font-semibold text-[#303030]">
                        ...
                        <div class="overflow-x-auto rounded-lg border">
                          <table class="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody v-if="SelectQueue.length > 0">
                              <tr
                                v-for="(item, index) in sortedQueueme.slice(
                                  0,
                                  5
                                )"
                                :key="item._id"
                      ...
                      <div
                        ...
                        <form action="#">
                          ...
                          <div
                            class="flex items-center content-center space-x-4">
                            <button
                              @click="addQueue_Btn(SelectAddModal)"
                              type="button"
                              class="text-[#140A4B] inline-flex items-center hover:text-white border border-[#140A4B] hover:bg-[#140A4B] focus:ring-4 focus:outline-none focus:ring-[#140A4B] font-medium rounded-lg text-sm px-5 py-2.5 text-center">
                              <svg
                                class="mr-1 -ml-1 w-5 h-5"
          ...
          <nav
            ...
            <ul class="inline-flex items-stretch -space-x-px">
              ...
              <li v-for="page in displayedPages" :key="page">
                <a
                  @click="goToPage(page)"
                  :class="{ 'aria-current': page === currentPage }"
              ...
              <li>
                <button
                  @click="movePage(1)"
                  :disabled="endIndex >= Queue.length"
